    ...
    <div class="tab_content" id="tab6">  <%-- 리뷰 --%>
      <div class="xans-product-detail">
        ...
        <div id="comment_add">
          ...
          <%-- 댓글 목록 출력 영역 --%>
          <div class="title">
            <p style="float: left; margin-top: 6em; margin-bottom: 0;">리뷰 목록</p>
          </div>
          <div id="comment_list">
          </div>
        </div>
...
<script type="text/javascript">
            ...
            success: function (commentArray) {
                ...
                $(commentArray).each(function () {
                    ...
                    $("#comment_list").append("<div id='comment_" + this.comId + "' class='comment' num='" + this.comId + "'>" +
                        "<p><h3>[ " + this.memberId + " ]</h3>" + this.regDate +
                        '</p>' +
                        '<div id ="comContent">' + this.comContent.replace(/\n/g, "<br>") + "</div>" +
                        "<button class='delete_btn'>삭제</button><button class='edit_btn'>수정</button></div>");
                });
            },
            error: function () {
                alert("list 로드시 에러발생");
    ...
    $("#comment_list").on("click", "button:contains('수정')", function () {
        ...
        $(this).parent().html("<textarea id='editContent' rows='3' cols='50'>" + comContent + "</textarea><br><br>" +
            "<button id='cancel_btn'>취소</button><button id='save_btn'>저장</button>");
