...
@WebServlet("/teachercommunity/write.do")
@MultipartConfig(
...
public class WriteController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        request.getRequestDispatcher("/view/board/teachercommunity/write.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        int result = dao.tcinsertWrite(dto);
        if (result == 1) {
            response.sendRedirect(request.getContextPath() + "/teachercommunity/list.do");
        } else {
            System.out.println("글쓰기 실패");
            JSFunction.alertLocation(response, "글쓰기에 실패했습니다.", request.getContextPath() + "/teachercommunity/write.do");
        }
    }
}
